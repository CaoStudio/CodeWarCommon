...
func sizeOfFaster(v reflect.Value, cache map[uintptr]bool) uint64 {
	switch v.Kind() {
	case reflect.Array:
		var sum uint64 = 0
		for i := 0; i < v.Len(); i++ {
			s := sizeOfFaster(v.Index(i), cache)
			if s < 0 {
				return 0
			}
			sum += s
	...
	case reflect.Slice:
		...
		return sum + uint64(unsafe.Sizeof(v))
	case reflect.Struct:
		var sum uint64 = 0
		for i, n := 0, v.NumField(); i < n; i++ {
			s := sizeOfFaster(v.Field(i), cache)
			if s < 0 {
				return 0
			}
			sum += s
	...
	case reflect.Ptr:
		...
		s := sizeOfFaster(reflect.Indirect(v), cache)
		if s < 0 {
			return 0
		}
	...
	case reflect.Map:
		...
		for i := range keys {
			...
			sv := sizeOfFaster(val, cache)
			if sv < 0 {
				return 0
			}
			sum += sv
			sk := sizeOfFaster(keys[i], cache)
			if sk < 0 {
				return 0
			}
			sum += sk
	...
	case reflect.Interface:
		return sizeOfFaster(v.Elem(), cache) + uint64(v.Type().Size())
